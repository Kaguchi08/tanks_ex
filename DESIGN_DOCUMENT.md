...
- **Blazor Server**: リアルタイム管理画面フレームワーク
- **Bootstrap 5**: 管理画面UIフレームワーク
...
- **プレゼンテーション層**: Unity UI とゲームビジュアル、Blazor管理画面
...
本プロジェクトには、ゲームサーバーの監視・管理を行うためのBlazor Server管理画面が統合されています。

#### 9.1.1. システム構成
- **フレームワーク**: Blazor Server (.NET 8)
- **UIライブラリ**: Bootstrap 5
- **レンダリングモード**: InteractiveServer
- **通信**: SignalR（双方向リアルタイム通信）
...
#### 9.2.1. ダッシュボード (`/`)
- **リアルタイム統計**: アクティブプレイヤー数、総接続数、サーバー稼働時間
- **ゲーム制御**: ゲーム開始/停止/リセット機能
- **サーバー情報**: システム状態、ゲーム準備状況
- **アクティビティログ**: 最近の操作履歴（最大10件）

#### 9.2.2. プレイヤー管理 (`/admin/players`)
- **プレイヤー一覧**: 接続中プレイヤーの詳細表示
- **アバター表示**: プレイヤー名の視覚的表示
- **プレイヤー操作**: 詳細表示、キック機能
- **リアルタイム更新**: 接続状態の自動更新

#### 9.2.3. リアルタイム監視 (`/admin/monitor`)
- **自動更新機能**: 1-10秒間隔での設定可能な自動更新
- **ライブメトリクス**: リアルタイムでの統計値更新
- **アクティビティフィード**: システム変更の自動検出とログ
- **システム状態**: MagicOnion、Blazor各サービスの稼働監視

#### 9.2.4. システムテスト (`/admin/test`)
- **接続テスト**: MagicOnion/Blazor接続確認
- **JSON変換テスト**: データシリアライゼーション検証
- **リアルタイム更新テスト**: Timer機能とUI更新確認
- **システム情報取得**: サーバー環境の詳細表示

### 9.3. 技術実装

#### 9.3.1. モダンなBlazor Server構造
- **.NET 8対応**: 最新のBlazor Server機能を活用
- **コンポーネントベース設計**: 再利用可能なRazorコンポーネント
- **InteractiveServer RenderMode**: リアルタイム双方向通信
- **Scoped CSS**: コンポーネント固有のスタイリング

#### 9.3.2. スレッドセーフティ
```csharp
// Timer内でのスレッドセーフなUI更新
...
#### 9.3.3. 統合アーキテクチャ
- **MagicOnion + Blazor併用**: HTTP/1.1とHTTP/2の適切な使い分け
- **サービス統合**: 既存の`PlayerManagerService`と`GameStateService`拡張
- **型安全性**: 共有ライブラリによるクライアント・サーバー間の型整合

### 9.4. ファイル構造
```
src/tanks.Server/
├── Components/
│   ├── App.razor                    # メインアプリケーション
│   ├── Routes.razor                 # ルーティング設定
│   ├── Layout/
│   │   ├── MainLayout.razor         # メインレイアウト
│   │   ├── MainLayout.razor.css     # レイアウト専用CSS
│   │   ├── NavMenu.razor            # ナビゲーションメニュー
│   │   └── NavMenu.razor.css        # メニュー専用CSS
│   └── Pages/
│       ├── Admin.razor              # 管理ダッシュボード
│       ├── Players.razor            # プレイヤー管理
│       ├── Monitor.razor            # リアルタイム監視
│       ├── Test.razor               # システムテスト
│       └── Error.razor              # エラーページ
└── wwwroot/
    ├── app.css                      # アプリケーション共通CSS
    └── bootstrap/
...
- **gRPCエンドポイント**: http://localhost:5001
...
- **管理画面監視**: Blazor Server稼働状況、リアルタイム更新機能
...
### 11.5. 管理システム設計 (A+)
- **Blazor Server統合**: MagicOnionとの完全な共存
- **リアルタイム監視**: 自動更新による動的監視機能
- **モダンUI**: Bootstrap 5とInteractiveServerによる高品質UX
...
7. **Blazor管理画面統合**: プロダクション対応のリアルタイム監視・管理システム
...
- **ハイブリッドサーバー**: MagicOnion（gRPC）とBlazor Server（HTTP/1.1）の共存
- **スレッドセーフUI**: Timer処理でのInvokeAsyncによる安全なUI更新
